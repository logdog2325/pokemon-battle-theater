...
#define B_BATTLE_BOND               GEN_8 // v2.1.0: pre-Gen9 behavior so Ash's Greninja TRANSFORMS into Ash-Greninja after its first KO (anime moment) instead of the Gen9 stat-boost. Only Ash's Greninja uses Battle Bond, so effectively Ash-specific. Set GEN_LATEST to revert.
...
#define B_FLAG_AI_VS_AI_BATTLE      FLAG_UNUSED_0x020 // Battle Simulator: when set, player side is AI-controlled.
...
// v1.1 — Battle Theater is a pure sim, the curtain slide-in is just overhead
// between matches. Skipping it shaves ~1s off every battle start.
...
// v1.1 — halved from vanilla 16. Battle Theater is mostly watching AI-vs-AI,
// fewer dialogue beats and shorter ones make the matches actually watchable
// instead of dragging. Lower than 8 starts to clip dialogue readability.
...
#define B_AFFECTION_MECHANICS           FALSE      // Battle Simulator v0.44: disabled — affection perks (endure on lethal hit / "toughed it out so you wouldn't feel sad", status cure on end-of-turn, crit/accuracy boosts) trigger off high friendship in pokeemerald-expansion, and Bianca's Happiness:255 mons were surviving fatal hits. AI-vs-AI sim doesn't want any of these.
...
// v2.0.5.2: Enable Showdown-style clauses for sim pool trainers (Noland,
// Argenta, Thorton, all _BT trainers). Each fight now samples 6 mons with
// no duplicate species, no duplicate held items, and at most one mega stone
...
// v2.0.5.5: reverted v2.0.5.4 — user prefers form variants allowed to coexist
// in the same party (Ninetales + Ninetales-Alola, Marowak + Marowak-Alola
// etc.). TRUE = exclude forms from the species clause = forms can coexist.
// Strict species-enum match still catches actual duplicates (two Turtonators
// with the same SPECIES enum value); only form variants are exempt.
#define B_POOL_RULE_EXCLUDE_FORMS           TRUE     // Forms allowed to coexist (Ninetales + Ninetales-Alola both fine)
